...
class Work_DB:
    ...
    def create_user(self, user_id, data_reg, date_last_use):
        with self.connection:
            return self.cursor.execute("INSERT INTO 'users' (user_id, date_reg, date_last_use)  VALUES  (?,?,?)", (user_id, data_reg, date_last_use))
    ...
    def update_date_use(self, user_id, date_use):
        with self.connection:
            return self.cursor.execute("UPDATE `users` SET `date_last_use` = ? WHERE `user_id` = ?", (date_use, user_id))
